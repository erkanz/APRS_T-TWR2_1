#!/usr/bin/env python3
"""Dependency-free ST7789 entry point for T-TWR Rev2.1.

Uses the ESP32 Arduino SPI core directly, avoiding the Adafruit ST7789 package's
optional seesaw/SD dependency chain that shadows the ESP32 framework SD library.
"""

import rev21_st7789_display as base
import rev21_st7789_display_v2 as webfix
...
def patch_platformio() -> None:
    text = base.PIO.read_text(encoding="utf-8")
    # The native SPI driver needs no new dependency. Remove a generated dependency
    # if this pass is ever applied after the older Adafruit-based prototype.
    lines = [line for line in text.splitlines() if "Adafruit ST7735 and ST7789 Library" not in line]
    base.PIO.write_text("\n".join(lines) + "\n", encoding="utf-8")


def main() -> None:
    base.HEADER.write_text(HEADER_CONTENT, encoding="utf-8")
    base.patch_main_h()
    base.patch_main()
    webfix.patch_web()
    patch_platformio()

    print("PASS native ST7789 240x320 support installed on official T-TWR expansion SPI pins")
    print("PASS ST7789 support introduces no third-party TFT/SD dependency")
